...
t=np.zeros(np_angle_ds.shape[0],dtype='float32')
for i in range(np_angle_ds.shape[0]):
    t[i]=(np_angle_ds[i,3]-start_sec)+(np_angle_ds[i,4]-start_nsec)/1000000000.0

ax1=plt.subplot(3,1,1)
plt.plot(t,np_angles[:np_angle_ds.shape[0],0],'r',linewidth=0.5)
plt.plot(t,np_angle_ds[:,0],'k',linewidth=0.5)
plt.ylabel('Roll(rad)')
plt.ylim([-0.5,0.5])
...
plt.plot(t,np_angles[:np_angle_ds.shape[0],1],'g',linewidth=0.5)
plt.plot(t,np_angle_ds[:,1],'k',linewidth=0.5)
...
plt.plot(t,np_angles[:np_angle_ds.shape[0],2],'b',linewidth=0.5)
plt.plot(t,np_angle_ds[:,2],'k',linewidth=0.5)
plt.xlabel('time(s)')
plt.ylabel('Yaw(rad)')
plt.ylim([-0.5,0.5])
plt.grid(True)
